...
const SITE_CONFIG = {
  ...
  */
  ...
  /* ------------------------------------------------------------
     ⑤-B 点餐页顶部常驻提示（不会被关闭，用于长期有效的优惠说明；
     临时公告请用上面①的 announcement）
     ------------------------------------------------------------
  ...
  mainDishRiceOffer: {
    enabled: true,
    text: "🍚 Offert : un riz nature (N°79B) gratuit pour chaque plat principal commandé !"
  },

  /* ------------------------------------------------------------
     ⑥-B 秋季火锅推广（首页宣传板块 + 预约弹窗里的提示语）
     ------------------------------------------------------------
     enabled          : true = 首页显示火锅板块    false = 隐藏（连同预约弹窗里的"火锅模式"提示语一起隐藏，
                        但预约功能本身仍然保留，只是不再特别提火锅）
     pricePerPerson   : 每人价格（数字）
     note             : 价格说明（法语，会显示在首页和预约弹窗里）
     ------------------------------------------------------------ */
  fondue: {
    enabled: true,
    pricePerPerson: 38,
    note: "Formule self-service à volonté, boissons non comprises."
  },

  /* ------------------------------------------------------------
     ⑥-C 订位预约设置
     ------------------------------------------------------------
     enabled                 : true = 首页显示"Réserver"预约功能
     maxDaysAhead            : 最多可以提前多少天预约
     leadTimeMinutes         : 如果预约"今天"，最早只能约从现在起多少分钟以后
     lastSeatingBeforeClose  : 打烊前多少分钟停止接受预约（留出用餐时间）
     maxPeople               : 下拉菜单最多可选多少人（超过这个人数，客人会在备注里说明）
     endpoint                : 预约接收地址，留空则跟上面②的餐厅邮箱一样用打开邮件的方式；
                                填 Formspree 地址则自动发送，默认跟在线订单用同一个地址
     ------------------------------------------------------------ */
  reservation: {
    enabled: true,
    maxDaysAhead: 30,
